...
* Remove unusued code in biblio.pm (https://bugs.launchpad.net/evergreen/+bug/2063980[Bug 2063980])
...
More fields have been marked required.  Required fields are that those that come from a database table, have a "NOT NULL" constraint in the schema, and do not have a default value assigned in the database.
...
Adds the following new Library Settings for example and/or regex validation for the default and SMS phone fields on the patron registration and edit interfaces (https://bugs.launchpad.net/evergreen/+bug/2035396[Bug 2035396]):
...
* Improves the accessiblity of the Angular staff login page (https://bugs.launchpad.net/evergreen/+bug/1839364[Bug 1839364])
