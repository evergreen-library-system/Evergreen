...
~~~~~~~~~~~~~~~~~~~~~~
  ...
  * **Linux**: Evergreen 3.2.8 has been tested on Debian Stretch (9.0),
...
. As the *opensrf* user, download and extract Evergreen 3.2.8:
+
[source, bash]
-----------------------------------------------
wget https://evergreen-ils.org/downloads/Evergreen-ILS-3.2.8.tar.gz
tar xzf Evergreen-ILS-3.2.8.tar.gz
-----------------------------------------------
+
[NOTE]
...
[source, bash]
---------------------------------------------
cd /home/opensrf/Evergreen-ILS-3.2.8
---------------------------------------------
+
...
[source, bash]
----------------------------------------------------------
cp /home/opensrf/Evergreen-ILS-3.2.8/Open-ILS/examples/apache/eg_startup /etc/apache2/eg_startup
----------------------------------------------------------
+
...
[source, bash]
----------------------------------------------------------
cp /home/opensrf/Evergreen-ILS-3.2.8/Open-ILS/examples/apache/eg_vhost.conf /etc/apache2/eg_vhost.conf
----------------------------------------------------------
+
...
[source, bash]
----------------------------------------------------------
cp /home/opensrf/Evergreen-ILS-3.2.8/Open-ILS/examples/apache/eg.conf /etc/apache2/sites-available/eg.conf
----------------------------------------------------------
...
the schema. For example, if you want to upgrade from 3.0.0 to 3.2.8, you
...
- 3.1.1-3.1.2-upgrade-db.sql
- 3.1.2-3.1.3-upgrade-db.sql
- 3.1.3-3.1.4-upgrade-db.sql
...
- 3.2.7-3.2.8-upgrade-db.sql


Note that you do *not* want to run additional 3.0 scripts to upgrade to the
newest version of 3.0, since currently there is no automated way to upgrade
from 3.0.7+ to 3.1. Only upgrade as far as necessary to reach the major
version upgrade script (in this example, as far as 3.0.6).

To upgrade across multiple major versions (e.g. from 2.12.0 to 3.2.8), use
the same logic to utilize the provided major version upgrade scripts. For
example:

- 2.12.6-3.0.0-upgrade-db.sql
- (run all incremental scripts from 3.0.0 to 3.0.6)
- 3.0.6-3.1.0-upgrade-db.sql
- (run all incremental scripts from 3.1.0 to 3.1.5)
- 3.1.5-3.2.0-upgrade-db.sql
- (run all incremental scripts from 3.2.0 to 3.2.8)
...
[source, bash]
----------------------------------------------------------
cd /home/opensrf/Evergreen-ILS-3.2.8/Open-ILS/src/sql/Pg
...
psql -U evergreen -h localhost -f version-upgrade/3.0.2-3.0.3-upgrade-db.sql evergreen
...
psql -U evergreen -h localhost -f version-upgrade/3.0.5-3.0.6-upgrade-db.sql evergreen
...
psql -U evergreen -h localhost -f version-upgrade/3.2.7-3.2.8-upgrade-db.sql evergreen
----------------------------------------------------------
