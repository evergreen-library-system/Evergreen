...
== Software Prerequisites ==
  ...
  * **Linux**: Evergreen 3.12.0 has been tested on Debian Buster
...
* **OpenSRF**: The minimum supported version of OpenSRF is 3.3.0. If you are upgrading from an older version of Evergreen using a previous version of OpenSRF, the OpenSRF upgrade should be performed first.
...
. As the *opensrf* user, download and extract Evergreen 3.12.0:
+
[source, bash]
-----------------------------------------------
wget https://evergreen-ils.org/downloads/Evergreen-ILS-3.12.0.tar.gz
tar xzf Evergreen-ILS-3.12.0.tar.gz
-----------------------------------------------
+
[NOTE]
...
[source, bash]
---------------------------------------------
cd /home/opensrf/Evergreen-ILS-3.12.0
---------------------------------------------
+
...
------------------------------------------------------------
cd /home/opensrf/Evergreen-ILS-3.12.0
make STAFF_CLIENT_STAMP_ID=rel_3.12.0 install
------------------------------------------------------------
+
...
rm server
ln -sf rel_3.12.0/server server
------------------------------------------------------------
+
...
[source, bash]
----------------------------------------------------------
cp /home/opensrf/Evergreen-ILS-3.12.0/Open-ILS/examples/apache/eg_startup /etc/apache2/eg_startup
----------------------------------------------------------
+
...
[source, bash]
----------------------------------------------------------
cp /home/opensrf/Evergreen-ILS-3.12.0/Open-ILS/examples/apache/eg_vhost.conf /etc/apache2/eg_vhost.conf
----------------------------------------------------------
+
...
[source, bash]
----------------------------------------------------------
cp /home/opensrf/Evergreen-ILS-3.12.0/Open-ILS/examples/apache/eg.conf /etc/apache2/sites-available/eg.conf
----------------------------------------------------------
...
the schema. For example, if you want to upgrade from 3.11.0 to 3.12.0, you
would run the following upgrade scripts:

- 3.11.0-3.11.1-upgrade-db.sql
- 3.11.1-3.12.0-upgrade-db.sql (this is a major version upgrade)
...
example to upgrade from 3.11.0 to 3.12. Only upgrade as far as necessary to
reach the major version upgrade script (in this example, as far as 3.11.1).
...
[source, bash]
----------------------------------------------------------
cd /home/opensrf/Evergreen-ILS-3.12.0/Open-ILS/src/sql/Pg
psql -U evergreen -h localhost -f version-upgrade/3.11.1-3.12.0-upgrade-db.sql evergreen
----------------------------------------------------------
