...
* Create Purchase Order From Selected Line Items
* Deleted Selected
...
When a selection list is cloned, the new list’s creator, owner, editor, organizational unit, etc. are set to the current user and workstation.
...
The search entry box will display a controlled list of values via a dropdown menu when available. You will see this when searching for an organizational unit, owner, state, status, and provider, among others. Search is case-insensitive.
...
. Searches based on an organizational unit value, such as _Selection List - Org Unit_ and _Purchase Order - Ordering Agency_ search only the organizational unit selected by default and do not use inheritance in the search. If search results spanning multiple branches/systems are needed, then a broader search can be performed. The search results can be filtered using the relevant organizational unit column and can be adjusted in the filter to include ancestor and/or descendant organizational units
