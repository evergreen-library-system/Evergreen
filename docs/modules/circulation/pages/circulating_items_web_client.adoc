...
Throughout Evergreen, you'll find a *Strict Barcode* checkbox next to the input field. When the checkbox is checked Evergreen will alert with a bad barcode message anytime a barcode that is not 14 digit codabar is scanned. Library staff are given an option to accept and continue using the bad barcode. This is used in situations where library staff want to capture items with barcodes that are not 14 digit codabar.
