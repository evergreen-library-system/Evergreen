...
image::circulating_items_web_client/renew_specific_due_date.png[Renew with a specific due date.]
...
2) Select the item. Click on *Actions* -> Under *Mark* select *Item as Damaged* or *Item as Missing*.

image::circulating_items_web_client/item_status_mark_damaged_missing.png[Mark item as Damaged or Missing in Item Status.]
...
The *Edit Item Attributes* function on the *Actions* dropdown list allows you to edit item records. Here, we will show you how to insert item alert messages by this function. See cataloging instructions for more information on item editing.

1) Retrieve record on *Item Status* screen.

2) Once item is displayed, highlight it and select *Actions* -> under *Edit* select *Call Numbers and Items*.

3) The item record is displayed in the *Holdings Editor*.  Click on the *Items Attributes* tab.

4) Under the *Misellaneous* column, select *Item Alerts*.

image::circulating_items_web_client/item_alerts_button.png[Item Alerts button.]

5) Select the appropriate options for *Select Alert Type* and type in your alert in the *New Alert Note* box.  Select *Temporary?* if you only want the alert showing once.

image::circulating_items_web_client/managing_item_alerts_prompt.png[Managing Item Alerts prompt.]

6) Select *Add New* then *Apply Changes* when done.
