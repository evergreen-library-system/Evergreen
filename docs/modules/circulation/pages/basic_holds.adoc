...
2. Click *Place Hold* either in the search results or within the detailed bibliographic record. The Place Hold screen will appear. Note: this feature also appears when placing volume-level holds and item-level holds.
+
...
+
4. A dialog box will appear with the patron search interface used elsewhere in the staff client. By default, the search scopes to your workstation organizational unit, and you can search by patron last name, first name, and middle name.
+
...
. The _Clear These Holds_ button becomes enabled when viewing clearable holds. Click it and the expired holds will be canceled. (Note that if your organization has enabled the "Hide the Clear These Holds button" xref:admin:librarysettings.adoc#lse-gui[library setting], the _Clear These Holds_ button will not display.)
