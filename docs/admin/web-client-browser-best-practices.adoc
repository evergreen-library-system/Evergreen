...
To see a list of keyboard shortcuts for web client functions, type ? into a web client window. A screen overlay will appear with the list of shortcuts. This will not work if your cursor is focused in an entry box, so make sure to click your mouse outside of an entry box.
