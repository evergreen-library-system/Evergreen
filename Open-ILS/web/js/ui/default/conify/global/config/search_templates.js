...
function populateTemplate(template)
	{
		...
		template.lang = getSelectedValues(langSel);
		template.audn = getSelectedValues(audnSel);
		template.type = getSelectedValues(typeSel);
...
function getGlobalRowValues(){
	...
	for (var i=0; i<rows.length; i++)
		{	
			...
			var selects = tds[0].getElementsByTagName("select");
			for (var s = 0; s<selects.length; s++)
			{			
				gRows[i][selects[s].name] = selects[s].value; 	
			}
...
function populateSearchOptions(template) {
	selectOptionValues(langSel, template.lang);
	selectOptionValues(audnSel, template.audn);
	selectOptionValues(typeSel, template.type);
...
function setGlobalRowValues(template){
	...
	for (var i=0; i<rows.length; i++)
		{	
			...
			for (var s = 0; s<selects.length; s++)
			{	
				selectOptionValue(selects[s], gRow[selects[s].name]);	
				if (selects[s].name == "qtype")
					{
						updateBeginsWithOption(selects[s], selects[s+1]);
					}
			}
		}	
	}
