...
closedir $dir;
my @modern_upgrade_scripts = grep is_modern_upgrade_script($_), sort @upgrade_scripts;
...
foreach(@modern_upgrade_scripts) {
    ...
    while (my $line = <$fh>) {
        if ($line =~ /^\s*SELECT evergreen.upgrade_deps_block_check.*:eg_version/i) {
            $found = 1;
        }
    }
    ...
    unless ($found) {
        my ($rev) = /^(\d+)\./;
        if ($rev && exists($exceptions{$rev})) {
            $known_exception = 1;
        }
    }

    ok($found || $known_exception, "Upgrade script $_ has a valid upgrade deps block check");
