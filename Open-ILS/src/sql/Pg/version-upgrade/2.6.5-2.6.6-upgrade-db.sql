--Upgrade Script for 2.6.5 to 2.6.6
\set eg_version '''2.6.6'''
BEGIN;
INSERT INTO config.upgrade_log (version, applied_to) VALUES ('2.6.6', :eg_version);
...
\qecho
\qecho
\qecho **** Certain improvements in this upgrade require a partial reingest of
...
\qecho
\qecho
\qecho **** then running it via psql, e.g.:
\qecho
\qecho
\qecho '\\i /tmp/partial_reingest_bib_recs.sql'
\qecho
\qecho
\qecho **** On a large database this could take hours or days to complete. If
\qecho **** that is your situation, consider splitting the file and running
\qecho **** multiple files in parallel, or using this as a guideline for another
\qecho **** solution altogether (e.g. pingest.pl).
