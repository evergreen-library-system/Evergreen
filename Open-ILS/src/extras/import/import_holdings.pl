...
my ($userid, $sourceid, $cn_id, $cp_id, $cp_file, $cn_file, $map_file) = (1, 2, 1, 1, 'asset_copy.sql','asset_volume.sql','record_id_map.pl');
...
COPY asset.copy (id,editor,creator,barcode,call_number,copy_number,status,home_lib,loan_duration,fine_level,circulate,deposit,deposit_amount,price,ref,opac_visible) FROM STDIN;
SQL
...
while ( $xml .= <STDIN> ) {
	...
	for my $node ($doc->documentElement->findnodes('/*/*[@tag="999"]')) {
		...
		unless (exists($$cn_map{"$rec_id/$home_lib/$label"})) {
			$$cn_map{"$rec_id/$home_lib/$label"} = $cn_id;
		...
		print CP join("\t", (	$cp_id,$userid,$userid,$barcode,
					$$cn_map{"$rec_id/$home_lib/$label"},
					$copy_number,1,$home_lib,2,2,1,0,'0.00',
					$price,0,1 )
			 )."\n";
		print 'c';
		$cp_id++;
